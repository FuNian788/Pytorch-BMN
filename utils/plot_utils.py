...
def plot_result(opt, num_proposals, recall, average_recall, tiou_threshold=np.linspace(0.5, 0.95, 10)):
    ...
    Arguements:
        ...
        num_proposals: (int[100]): number of selected proposals per video, which is average number of proposals(range from 1 to 100).
    ...
    # AR(Calculate the integral result while x-demension is num_proposals and y-demension is recall, or area size below the curve).
